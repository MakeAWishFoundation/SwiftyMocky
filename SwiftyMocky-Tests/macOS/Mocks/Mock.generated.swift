...
open class InoutProtocolMock: InoutProtocol, Mock {
    ...
    open class Given: StubbedMethod {
        ...
        public static func returnAndInOut(value: Parameter<Int>, willReturn: String...) -> MethodStub {
            return Given(method: .m_returnAndInOut__value_value(`value`), products: willReturn.map({ Product.return($0) }))
        }
        public static func returnAndInOut(value: Parameter<Int>, willProduce: (Stubber<String>) -> Void) -> MethodStub {
            let willReturn: [String] = []
			let given: Given = { return Given(method: .m_returnAndInOut__value_value(`value`), products: willReturn.map({ Product.return($0) })) }()
			let stubber = given.stub(for: (String).self)
			willProduce(stubber)
			return given
        }
...
// MARK: - ProtocolWithGenericsAndConstraints
open class ProtocolWithGenericsAndConstraintsMock: ProtocolWithGenericsAndConstraints, Mock {
    init(sequencing sequencingPolicy: SequencingPolicy = .lastWrittenResolvedFirst, stubbing stubbingPolicy: StubbingPolicy = .wrap, file: StaticString = #file, line: UInt = #line) {
        self.sequencingPolicy = sequencingPolicy
        self.stubbingPolicy = stubbingPolicy
        self.file = file
    ...
    open func save<T: Codable>(data: T) throws {
        addInvocation(.m_save__data_data(Parameter<T>.value(`data`).wrapAsGeneric()))
		let perform = methodPerformValue(.m_save__data_data(Parameter<T>.value(`data`).wrapAsGeneric())) as? (T) -> Void
		perform?(`data`)
		do {
		    _ = try methodReturnValue(.m_save__data_data(Parameter<T>.value(`data`).wrapAsGeneric())).casted() as Void
		} catch MockError.notStubed {
			// do nothing
		} catch {
		    throw error
    ...
    fileprivate enum MethodType {
        case m_save__data_data(Parameter<GenericAttribute>)
        case m_getData
    ...
    open class Given: StubbedMethod {
        ...
        public static func getData<T: Codable>(willReturn: T?...) -> MethodStub {
            return Given(method: .m_getData, products: willReturn.map({ Product.return($0) }))
        }
        public static func getData<T: Codable>(willProduce: (Stubber<T?>) -> Void) -> MethodStub {
            let willReturn: [T?] = []
			let given: Given = { return Given(method: .m_getData, products: willReturn.map({ Product.return($0) })) }()
			let stubber = given.stub(for: (T?).self)
			willProduce(stubber)
			return given
        }
        public static func save<T: Codable>(data: Parameter<T>, willThrow: Error...) -> MethodStub {
            return Given(method: .m_save__data_data(`data`.wrapAsGeneric()), products: willThrow.map({ Product.throw($0) }))
        }
        public static func save<T: Codable>(data: Parameter<T>, willProduce: (StubberThrows<Void>) -> Void) -> MethodStub {
            let willThrow: [Error] = []
			let given: Given = { return Given(method: .m_save__data_data(`data`.wrapAsGeneric()), products: willThrow.map({ Product.throw($0) })) }()
			let stubber = given.stubThrows(for: (Void).self)
			willProduce(stubber)
			return given
        }
    ...
    public struct Verify {
        ...
        public static func save<T>(data: Parameter<T>) -> Verify where T:Codable { return Verify(method: .m_save__data_data(`data`.wrapAsGeneric()))}
        public static func getData() -> Verify where T:Codable { return Verify(method: .m_getData)}
    }
    ...
    public struct Perform {
        ...
        public static func save<T>(data: Parameter<T>, perform: @escaping (T) -> Void) -> Perform where T:Codable {
            return Perform(method: .m_save__data_data(`data`.wrapAsGeneric()), performs: perform)
        }
        public static func getData(perform: @escaping () -> Void) -> Perform where T:Codable {
